...
public class CommentDao extends AbstractMFlixDao {
    ...
    public boolean updateComment(String commentId, String text, String email) {
        ...
        FindIterable<Comment> comments = commentCollection.find(Filters.eq("_id", objectId));
        Comment first = comments.first();
        if (!Objects.equals(first.getEmail(), email)) {
            throw new IncorrectDaoOperation("id doesnt match");
        }
        Document query = new Document("_id", objectId);

        UpdateResult updateResult = commentCollection.updateOne(query, set("text", text));
        // user own comments
        // TODO> Ticket - Handling Errors: Implement a try catch block to
        // handle a potential write exception when given a wrong commentId.
    ...
    public boolean deleteComment(String commentId, String email) {
        DeleteResult deleteResult = commentCollection.deleteOne(Filters.and(Filters.eq("_id", new ObjectId(commentId)), Filters.eq("email", email)));
    ...
    public List<Critic> mostActiveCommenters() {
        ...
        List<Bson> mostActivePipeline =
            Arrays.asList(group("$email", sum("count", 1L)), sort(descending("count")), limit(20));

        MongoCursor<Critic> iterator = commentCollection
            .withReadConcern(ReadConcern.MAJORITY)
            .aggregate(mostActivePipeline, Critic.class).iterator();
